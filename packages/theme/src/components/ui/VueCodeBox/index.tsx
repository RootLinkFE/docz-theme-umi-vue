...
import { ResizeBar } from '../Playground/ResizeBar'
import { ActionsBar } from '../Playground/ActionsBar'
import { Handle, HANDLE_SIZE } from '../Playground/Handle'
...
import hlStyle from './hljs.css'
import { definer as hljsDefineVue } from 'highlightjs-vue'
SyntaxHighlighter.registerLanguage('vue', hljsDefineVue)

interface OverlayProps {
  full: boolean
}

const whenFullscreen = (on: any, off: any) => (p: OverlayProps) => (p.full ? on : off)
...
export const VueCodeBox: React.FC<PlaygroundProps> = props => {
  const {
    position,
    code: initialCode,
    codesandbox,
  ...
  const vuePreviewId = React.useRef(Date.now())
  const isMount = React.useRef(false)
  React.useLayoutEffect(() => {
    if (language === 'vue' && !isMount.current) {
      new Vue({ render: createElement => createElement(component) }).$mount(
        '#vue-preview-' + vuePreviewId.current
      )
        ...
        <Wrapper full={fullscreen}>
          <PreviewWrapper full={fullscreen}>
            <StyledPreviewWrapper>
              <CustomWrapper>
                <div id={'vue-preview-' + vuePreviewId.current}></div>
              </CustomWrapper>
            </StyledPreviewWrapper>
            <StyledError />
          </PreviewWrapper>
          ...
          {showEditor && (
            <div className="hljs">
              <SyntaxHighlighter language="vue" style={hlStyle}>
                {code || '// empty code'}
              </SyntaxHighlighter>
            </div>
          )}
